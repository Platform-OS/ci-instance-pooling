...
export class RegistrationForm extends Form {
  ...
  constructor(page: Page) {
    ...
    this.nameToLocatorMapping = {
      ...
      token: { type: 'input', locator: '#pos-user-token' }
    };
...
export class RegistrationPage extends BasePage {
  ...
  async registerUser(user: { firstName: string, lastName: string, email: string; }, password: string, token: string) {
    await this.form.fillUserData({ ...user, password, token });
